...
Prior to Zerocash, anonymous electronic cash had been studied in centralised settings @chaum82 @chl05 and in decentralised settings via Zerocoin @mggr13. Centralised schemes assume a trusted bank to mint coins and prevent double-spending. Zerocoin removed the bank by leveraging zero-knowledge proofs over a public list of coin commitments stored on the Bitcoin ledger, but its proofs were large (~45 kB) and slow to verify (~450 ms). Concurrent work @dfkp13 also explored using zk-SNARKs in Zerocoin, but their construction has cost that grows superlinearly in the number of coins. Zerocash @zerocash differs in two key respects: it provides a full-fledged anonymous payment system rather than a mixer, and its proof and verification cost are essentially constant in the size of the anonymity set.
